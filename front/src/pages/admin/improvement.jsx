...
export default function ManagerDashboardImprovementPage() {
  // Next.js のルーターから URL のクエリパラメータを取得するために useRouter を利用
  ...
  // 選択された組織の名前（会社名・組織名）
  // 初期値は organizations[0].name だが、URLのクエリパラメータ (org) があれば上書きする
  const [selectedOrg, setSelectedOrg] = useState(organizations[0].name)

  // URLのクエリパラメータから selectedOrg を設定
  useEffect(() => {
    if (router.isReady && router.query.org) {
      setSelectedOrg(router.query.org)
    }
  ...
  useEffect(() => {
    ...
    const generateChartData = () => {
      // 組織に応じて少し数値を変える
      // selectedOrg は組織名なので、"組織AAA"、"組織ABA"、"組織AAB" で判定する
      const seedValue =
        selectedOrg === '組織AAA' ? 1.0 : selectedOrg === '組織ABA' ? 1.2 : 0.8
